...
void wall_clock::set_big_font()
{
	SDL_RWseek(font_source_, 0, RW_SEEK_SET);
	TTF_CloseFont(font_big_);
	font_big_ = TTF_OpenFontRW(font_source_, false, (width_ - ampm_width_) * height_ * 4 / 9 / (std::max(time_width_, width_) - ampm_width_));
	if (!font_big_)
	{
		throw "TTF_OpenFont";
	}
...
{
    return digit_width_ * 4 + colon_width_ + (seconds_ ? digit_width_ * 2 + colon_width_ : 0) + (time_24_ ? 0 : ampm_width_);
}
...
void wall_clock::redraw(const bool second_only)
{
	...
	if (!second_only)
	{
		...
		sTime <<
			std::setfill('0') << std::setw(2) << (time_24_ ? now_.tm_hour : now_.tm_hour % 12) << ":" <<
	...
	iX = (width_ - size_time_.x - (seconds_ ? size_second_.x : 0) - (time_24_ ? 0 : size_ampm_.x)) / 2;
	...
	if (!time_24_)
	{
		render_texture(texture_ampm_, &size_ampm_, iX, iY + (size_time_.y - size_ampm_.y) / 2);
		iX += size_ampm_.x;
	}
